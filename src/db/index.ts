...
export default async function getDatabase() {
  ...
  if (PG_POOL_ENABLED) {
    ...
    pool.on("connect", () =>
      log.debug(
        `pg:pool established a connection. open connection count is: ${pool.totalCount}`,
      ),
    );
    pool.on("acquire", () =>
      log.trace(
        `pg:pool acquierd a client to execute a query. waiting count: ${pool.waitingCount}`,
      ),
    );
    pool.on("release", () =>
      log.trace(
        `pg:pool released a client for fututre queries. waiting count: ${pool.waitingCount}`,
      ),
    );
    pool.on("remove", () =>
      log.debug(
        `pg:pool removed a connection. open connection count is: ${pool.totalCount}`,
      ),
    );
